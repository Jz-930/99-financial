...
export default function PlanningConceptsIndex() {
    const posts = getAllPosts('planning-concepts', [
        'title',
        'date',
        'slug',
        'excerpt',
        ...
        <div className="bg-brand-light min-h-screen">
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="grid grid-cols-1 gap-6">
                    {posts.map((post) => (
                        <Link key={post.slug} href={`/planning-concepts/${post.slug}`} className="group block">
                                ...
                                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                    <div className="flex-grow">
                                        ...
                                        <span className="inline-flex items-center text-brand-blue font-bold uppercase tracking-wider text-xs border-b-2 border-brand-gold/20 pb-1 group-hover:border-brand-gold transition-all">
                                            Read Explanation <i className="ml-2 fa-solid fa-arrow-right text-brand-gold group-hover:translate-x-1 transition-transform"></i>
                                        </span>
                                    </div>
                                </div>
                            </div>
