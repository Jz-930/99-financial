...
export async function generateStaticParams() {
    const posts = getAllPosts('foundational-articles', ['slug']);
    return posts.map((post) => ({
        slug: post.slug,
    }));
}

export default async function FoundationalArticle({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const post = getPostBySlug('foundational-articles', slug, [
        'title',
        'date',
        'slug',
        'content',
    ...
    return (
        <div className="bg-brand-light min-h-screen pt-12 pb-24">
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
                <Link href="/foundational-articles" className="text-slate-500 hover:text-brand-gold font-medium mb-8 inline-flex items-center text-sm uppercase tracking-wider transition-colors">
                    <i className="fa-solid fa-arrow-left mr-2"></i> All Foundational Articles
                </Link>
            </div>
                  ...
                  hover:prose-a:text-brand-gold hover:prose-a:no-underline prose-a:text-brand-blue prose-a:transition-colors"
                        ...
                        <div className="mt-16 pt-8 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <div className="hidden md:block">
                                <img src="/images/logo_origin.webp" alt="99 Financial" className="h-8 opacity-20 grayscale" />
                            </div>
                        </div>
                    </div>
                </div>
